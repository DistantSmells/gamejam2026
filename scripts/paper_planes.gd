...
func _physics_process(delta: float) -> void:
	$FlightPath/PathFollow2D.progress_ratio += (1 / time_to_traverse) * delta
	print($FlightPath/PathFollow2D/PathData.global_position)
	$PaperPlane.global_position = $FlightPath/PathFollow2D/PathData.global_position
	$PaperPlane.global_rotation = lerp($PaperPlane.rotation, $FlightPath/PathFollow2D/PathData.global_rotation, 0.5)
